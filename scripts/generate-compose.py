    ...
    if isinstance(data, str):
        def replacer(match):
            ...
            if var not in env_vars:
                print(f"Error: {var} is required but not set in .env.local")
                sys.exit(1)
            return env_vars[var]
...
def apply_env_overrides(compose: dict, env_name: str, env_vars: dict[str, str]) -> dict:
    ...
    # For non-dev, move internal service ports to x-ports (documentation only)
    if env_name != "dev":
        for svc_name in ("postgres", "minio", "server"):
            svc = services.get(svc_name)
            if svc is not None and "ports" in svc:
                svc["x-ports"] = svc.pop("ports")
...
def write_compose(env_name: str, compose: dict) -> Path:
    ...
    with open(filepath, "w", encoding="utf-8") as f:
        f.write(f"# {'=' * 50}\n")
        f.write(f"# Auto-generated by script, DON'T edit manually.\n")
        f.write("# Instead, modify the environment and re-run the script.\n")
        f.write(f"# - script     : scripts/generate-compose.py\n")
        f.write(f"# - environment: docker/.env.local\n")
        f.write(f"# - template   : docker/compose.base.yaml\n")
        f.write(f"# {'=' * 50}\n")
...
def main() -> int:
    ...
    # Load env vars from .env.local only
    ...
    if not env_vars:
        print(f"Error: {env_path} is empty or has no valid variables.")
        print("  Copy docker/.env.example to docker/.env.local and fill in the values.")
        sys.exit(1)
    ...
    if "DATABASE_URL" not in env_vars or not env_vars["DATABASE_URL"]:
        for key in ("POSTGRES_USER", "POSTGRES_PASSWORD", "POSTGRES_DB"):
            if key not in env_vars or not env_vars[key]:
                print(f"Error: {key} is required but not set in {env_path}")
                sys.exit(1)
