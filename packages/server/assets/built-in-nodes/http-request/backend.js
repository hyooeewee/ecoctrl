module.exports = async function execute(ctx, api) {
  ...
  if (!url) {
    api.log.error("[http-request] missing required field 'url'");
    throw new Error("http-request node requires 'url'");
  }

  const validMethods = ["get", "post", "put", "patch", "delete"];
  if (!validMethods.includes(method)) {
    api.log.error(`[http-request] invalid method: ${method}`);
    throw new Error(
      `http-request node invalid method: "${method}". Expected one of: GET, POST, PUT, PATCH, DELETE`,
    );
  }
  ...
  api.log.info(`[http-request] ${method.toUpperCase()} ${url} timeout=${timeoutMs}ms`);

  let response;
  try {
    response = await api.http[method](url, opts);
  } catch (err) {
    api.log.error(`[http-request] ${method.toUpperCase()} ${url} failed: ${err.message}`);
    throw new Error(`HTTP 请求失败 (${method.toUpperCase()} ${url}): ${err.message}`, {
      cause: err,
    });
  }

  api.log.info(`[http-request] ${method.toUpperCase()} ${url} -> status=${response.status}`);

  let parsedBody;
  try {
    parsedBody = response.json();
  } catch {
    api.log.warn(`[http-request] Response body is not valid JSON, returning raw body`);
