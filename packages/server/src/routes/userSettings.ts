...
export default async function userSettingsRoutes(fastify: FastifyInstance) {
  fastify.get(
    "/",
    {
      schema: {
        tags: ["Settings"],
        summary: "Get authenticated user dashboard settings",
  ...
  fastify.patch(
    "/",
    {
      schema: {
        tags: ["Settings"],
        summary: "Update authenticated user dashboard settings",
    ...
    async (request, reply) => {
      ...
      await upsertUserSettings(userId, body as Record<string, unknown>);
