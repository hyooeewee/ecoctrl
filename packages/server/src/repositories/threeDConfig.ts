...
export async function getThreeDConfig(): Promise<ThreeDConfig | null> {
...
export async function setThreeDConfig(config: ThreeDConfig): Promise<void> {
  const rows = await db.select().from(threeDConfigs).limit(1);
  if (rows.length === 0) {
    await db.insert(threeDConfigs).values({
