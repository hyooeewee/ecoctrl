...
export async function findBackupSchedule(): Promise<{ nextBackup: string } | null> {
  const rows = await db.select().from(backupSchedules).limit(1);
  if (rows.length === 0) return null;
  return { nextBackup: rows[0].nextBackup };
}

export async function updateBackupSchedule(nextBackup: string): Promise<{ nextBackup: string }> {
  const rows = await db.select().from(backupSchedules).limit(1);
  if (rows.length === 0) {
