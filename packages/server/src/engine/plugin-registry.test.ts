...
describe("PluginRegistry", () => {
  ...
  it("gets the latest version when no version specified", async () => {
    ...
    zip1.addFile("manifest.json", Buffer.from(JSON.stringify({
      id: "multi-version",
      name: "Multi Version",
      version: "1.0.0",
      category: "action",
    ...
    zip1.addFile("schema.json", Buffer.from(JSON.stringify({ type: "object", properties: { x: { type: "string" } } })));
    await registry.install(zip1.toBuffer());

    // Install v2.0.0
    const zip2 = new AdmZip();
    zip2.addFile("manifest.json", Buffer.from(JSON.stringify({
      id: "multi-version",
      name: "Multi Version",
      version: "2.0.0",
      category: "action",
    ...
    zip2.addFile("schema.json", Buffer.from(JSON.stringify({ type: "object", properties: { x: { type: "string" } } })));
  ...
  it("loads plugins from disk on loadAll", async () => {
    ...
    await fs.writeFile(path.join(pluginDir, "schema.json"), JSON.stringify({ type: "object", properties: { x: { type: "string" } } }));
  ...
  it("rejects install with hash mismatch", async () => {
    ...
    await expect(registry.install(zip.toBuffer())).rejects.toThrow("Package integrity check failed");
  });
});
