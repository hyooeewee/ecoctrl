    ...
    <BentoCard>
      ...
      <div className="scrollbar-hidden min-h-0 flex-1 overflow-y-auto px-3 pb-3 pt-2">
        <div className="flex flex-col gap-1">
          <DeviceItem icon={<IconWind size={14} />}         label={t.devices.airConditioning} count={6}  status="critical" />
          <DeviceItem icon={<IconBolt size={14} />}         label={t.devices.lighting}        count={30} status="warn"     />
          <DeviceItem icon={<IconElevator size={14} />}     label={t.devices.elevators}       count={10} status="ok"       />
          <DeviceItem icon={<IconDeviceLaptop size={14} />} label={t.devices.servers}         count={24} status="ok"       />
        </div>
      </div>
    </BentoCard>
  )
...
export function AlertsPanel() {
  return (
    <BentoCard>
      ...
      <div className="scrollbar-hidden min-h-0 flex-1 overflow-y-auto px-3 pb-3 pt-2">
        <div className="flex flex-col gap-1">
          <AlertItem icon={<IconTemperature size={12} />}   title={t.alerts.serverTempTitle}  subtitle={t.alerts.serverTempSub}  severity="critical" time="02:14" />
          <AlertItem icon={<IconWind size={12} />}          title={t.alerts.hvacWarnTitle}    subtitle={t.alerts.hvacWarnSub}    severity="warning"  time="05:32" />
          <AlertItem icon={<IconFlame size={12} />}         title={t.alerts.powerSurgeTitle}  subtitle={t.alerts.powerSurgeSub}  severity="warning"  time="12:07" />
          <AlertItem icon={<IconAlertTriangle size={12} />} title={t.alerts.upsBatteryTitle}  subtitle={t.alerts.upsBatterySub}  severity="critical" time="14:55" />
        </div>
      </div>
    </BentoCard>
  )
  ...
  return (
    <div className="flex items-start gap-2 rounded-lg border border-cyber-cyan/15 bg-cyber-cyan/5 px-2.5 py-2">
      ...
      <div className="min-w-0 flex-1">
        ...
        {saving && (
          <p className="mt-0.5 text-[9px] font-semibold text-cyber-green">↓ {saving}</p>
        )}
      </div>
    </div>
  )
    ...
    <BentoCard>
      ...
      <div className="scrollbar-hidden min-h-0 flex-1 overflow-y-auto px-3 pb-3 pt-2">
        <div className="flex flex-col gap-1.5">
          <SuggestionItem icon={<IconWind size={12} />}     text={t.ai.hvacText}     saving={t.ai.hvacSaving}     />
          <SuggestionItem icon={<IconBulb size={12} />}     text={t.ai.lightingText} saving={t.ai.lightingSaving} />
          <SuggestionItem icon={<IconActivity size={12} />} text={t.ai.serverText}   saving={t.ai.serverSaving}   />
        </div>
