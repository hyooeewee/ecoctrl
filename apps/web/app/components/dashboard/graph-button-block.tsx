...
export interface GraphButtonBlockProps {
  title: string;
  icon?: ReactNode;
  value: string;
  unit: string;
...
// ─── Large charts (modal detail view) ─────────────────────────────────────────

function LargeArea({ data, color }: { data: SparkPoint[]; color: string }) {
  const [ref, width] = useMeasureWidth(600);
  const chartData = data.map((d, i) => ({ ...d, t: i }));
  const gradId = `large-area-${color.replace(/[^a-z0-9]/gi, "x")}`;
  return (
...
function LargeBar({ data, color }: { data: SparkPoint[]; color: string }) {
  const [ref, width] = useMeasureWidth(600);
  const chartData = data.map((d, i) => ({ ...d, t: i }));
  return (
    <div ref={ref} className="h-[240px] w-full">
...
function LargeLine({ data, color }: { data: SparkPoint[]; color: string }) {
  const [ref, width] = useMeasureWidth(600);
  const chartData = data.map((d, i) => ({ ...d, t: i }));
  return (
    <div ref={ref} className="h-[240px] w-full">
...
function LargeProgress({ value, color }: { value: number; color: string }) {
  return (
    <div className="flex flex-col gap-6">
      <div className="flex items-end justify-between">
        <span className="font-heading text-5xl font-bold tabular-nums" style={{ color }}>
          {value}%
        </span>
      </div>
    ...
    >
      ...
      <CardContent className="relative flex h-full flex-col gap-1.5 px-3 py-2">
        ...
        <div className="shrink-0">
          {chartType === "area" && <MiniArea data={chartData} color={chartColor} />}
          {chartType === "bar" && <MiniBar data={chartData} color={chartColor} />}
          {chartType === "line" && <MiniLine data={chartData} color={chartColor} />}
          {chartType === "progress" && progressValue !== undefined && (
            <MiniProgress value={progressValue} color={chartColor} />
          )}
        </div>
        {/* ── Row 4: footer ── */}
...
// ─── Detail view (used inside modal) ──────────────────────────────────────────

export function GraphButtonBlockDetail({
  title,
  icon,
  value,
  unit,
    ...
    <div className="relative flex flex-col gap-6 p-8">
      ...
      <div className="relative flex items-start justify-between gap-4">
        <div className="flex items-center gap-3">
          {icon && (
            <span
              className="flex size-8 shrink-0 items-center justify-center rounded-lg bg-white/5 text-xl"
              style={{ color: chartColor }}
            >
              {icon}
            </span>
        ...
        {delta && (
          <span
            className={cn(
              "inline-flex shrink-0 items-center rounded-full border px-3 py-1 text-sm font-semibold",
              deltaStyle[deltaVariant],
            )}
          >
            {delta}
      ...
      <div className="relative">
        ...
        {chartType === "progress" && progressValue !== undefined && (
          <LargeProgress value={progressValue} color={chartColor} />
        )}
      </div>
