    ...
    <div className="space-y-6">
      ...
      <Tabs defaultValue="list" className="w-full">
        <TabsList className="bg-gray-100/80 p-1">
          <TabsTrigger value="list" className="gap-2 px-6 data-selected:bg-white data-selected:shadow-sm">
            <History size={16} />
            实时故障列表
          </TabsTrigger>
          <TabsTrigger value="analytics" className="gap-2 px-6 data-selected:bg-white data-selected:shadow-sm text-gray-500">
            <BarChart3 size={16} />
            故障统计分析
          </TabsTrigger>
        </TabsList>
                 ...
                 <TableBody>
                   {FAULTS.map((fault) => (
                     <TableRow key={fault.id}>
                        ...
                        <TableCell>
                           <Badge 
                            className={fault.level === '严重' ? 'bg-red-50 text-red-600' : fault.level === '一般' ? 'bg-orange-50 text-orange-600' : 'bg-blue-50 text-blue-600'}
                            variant="outline"
                           >
                             {fault.level}
                           </Badge>
                        </TableCell>
                        <TableCell className="text-gray-500 font-mono text-sm">{fault.time}</TableCell>
                        <TableCell className="text-right">
                           <span className={`text-sm font-medium ${fault.status === '待处理' ? 'text-red-500' : fault.status === '维保中' ? 'text-orange-500' : 'text-green-500'}`}>
                             {fault.status}
                           </span>
                        </TableCell>
                     </TableRow>
