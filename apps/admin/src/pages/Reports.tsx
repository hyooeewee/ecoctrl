    ...
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        ...
        <div className="flex items-center gap-3">
          <Button variant="outline" className="gap-2" onClick={() => console.log('Manage report templates...')}>
            <Settings2 size={16} />
            管理模板
          </Button>
          <Button variant="outline" className="gap-2" onClick={() => console.log('New scheduled report task...')}>
            <PlusCircle size={16} />
            新建定时任务
          </Button>
            ...
            <TableBody>
              {REPORT_PLANS.map((plan) => (
                <TableRow key={plan.id} className="group hover:bg-muted/30 transition-colors">
                  ...
                  <TableCell className="text-blue-600 underline underline-offset-4 cursor-pointer text-sm" onClick={() => console.log('Edit receiver for:', plan.name)}>
                    {plan.receiver}
                  </TableCell>
                  <TableCell className="text-gray-600 text-sm">{plan.frequency}</TableCell>
                  <TableCell className="text-right">
                    <Switch defaultChecked={plan.status} onCheckedChange={(checked) => console.log(`Switch plan ${plan.name} status to:`, checked)} />
                  </TableCell>
                </TableRow>
              ))}
            </TableBody>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        {[
          ...
          <Card key={i} className="border-none shadow-sm hover:translate-y-[-2px] transition-all cursor-pointer group relative overflow-hidden">
