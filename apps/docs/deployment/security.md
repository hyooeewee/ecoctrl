...
```mermaid
flowchart TB
    subgraph Network["ecoctrl_default (bridge 网络)"]
        Server["server:3000"]
        Postgres["postgres:5432 (内网)"]
        MinIO["minio:9000 (内网)"]
        Web["web:80 (Caddy)"]
        Admin["admin:80 (Caddy)"]

        Server ---|数据库连接| Postgres
        Server ---|对象存储| MinIO
        Server ---|反向代理| Web
        Server ---|反向代理| Admin
    end
    ...
    MinIO -- ":9000 (预签名URL)" --> Host
    Web -- ":8081" --> Host
    Admin -- ":4173" --> Host

    classDef host fill:#f0f0f0,stroke:#999,stroke-dasharray: 5 5
```
...
| 措施                     | 说明                                           |
| ------------------------ | ---------------------------------------------- |
| 最小暴露原则             | 仅向外暴露 admin(4173)、web(8081)、minio(9000) |
| 数据库不对外暴露         | PostgreSQL 仅在内部网络中可访问                |
| MinIO 文件访问           | 通过 Server 预签名 URL 授权访问                |
